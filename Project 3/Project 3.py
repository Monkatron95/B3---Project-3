...
def instructions():
    ...
    text_title=title_font.render("Instructions", True, (128,128,128))
    text_body=font.render("The purpose of this project was to create a program in which a robot has to collect different treasures within a minute. This can be aided or", True, (128,128,128))
    text_body2=font.render("hindered by the user through the interface provided. The user is able to:", True, (128,128,128))
    text_body3=font.render("- generate and move treasures by dragging them with the mouse", True, (128,128,128))
    text_body4=font.render("- modify a 'wishlist' for the robot in order to decide which type of treasure it should go for next", True, (128,128,128))
    text_body5=font.render("- increase or decrease the speed at which the robot is moving", True, (128,128,128))
    text_body6=font.render("- drop a bomb from a random coordonate, if that bomb hits the robot, its score it halved", True, (128,128,128))
    text_body7=font.render("The program also provides the user with various additional features such as:", True, (128,128,128))
    text_body8=font.render("- a music player", True, (128,128,128))
    text_body9=font.render("- a traffic light which stops or slows the robot down", True, (128,128,128))
    text_body10=font.render("- a pause function which stops the timer and the movement within the map", True, (128,128,128))
    screen.fill((200,200,200))
    screen.blit(text_title,[500,25])
    screen.blit(text_body,[50,100])
    screen.blit(text_body2,[50,130])
    screen.blit(text_body3,[150,160])
    screen.blit(text_body4,[150,190])
    screen.blit(text_body5,[150,220])
    screen.blit(text_body6,[150,250])
    screen.blit(text_body7,[50,300])
    screen.blit(text_body8,[150,330])
    screen.blit(text_body9,[150,360])
    screen.blit(text_body10,[150,390]) 
    while True:
        startButton.update(main, None)
        quitButton.update(terminate, None)
        checkForEvents(None)
